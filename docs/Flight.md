...
::: info
Visit [Cold Orbiting](/cold-orbiting) for more information and video demonstrations of this flight strategy.
:::
...
Upon entering the instance, just like any other Thargoid, there is a chance to avoid or atleast delay detection, hitting silent running in the first split seconds of dropping out will often work. If it is facing askew or the wrong way, then there’s more time to get a head start running. If your ship passes into alignment with the Glaive, it will gain an immediate target lock, if your ship has a detectable heat signature. The Glaive will maneuver into engagement distance to begin combat. If it gets within combat engagement range, it will fire its containment missile.

If your ship can maintain an average speed of 500 m/s then, with some luck and repeated boosts, the ship should be capable of keeping the Glaive just out of combat engagement range in order to safely wake-out, even when the glaive spotted the ship shortly after dropping.

If your ship is not capable of outrunning a glaive, then a contingency plan needs to be adopted, fight or die. If hardpoints are a flexible option, choose Enchanced AX multicannons (Pre-engineered are ideal) when operating near thargoid controlled space, to deal with hunters. Be aware that the Glaive has other special attacks that can be devastating to the unprepared. The ECM module can affect the FSD reboot missile, making running an option for the average speed ships.
